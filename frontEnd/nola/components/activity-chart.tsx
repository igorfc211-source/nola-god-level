...
export function ActivityChart() {
  ...
  useEffect(() => {
    async function fetchSales() {
      try {
        const res = await fetch("http://localhost:8000/sales?limit=300000")
  ...
  // 🔹 Processa os dados conforme o modo atual
  const chartData = useMemo(() => {
    const grouped: Record<string, number> = {}

    if (salesData.length === 0) return []

    // 📅 Caso um dia da semana esteja selecionado → mostra períodos (manhã, tarde, noite)
    if (selectedWeekDay) {
      salesData.forEach((sale) => {
        const date = new Date(sale.created_at)
        const weekDay = weekDays[date.getDay()]
        // compara só o nome do dia (sem a data)
    ...
    // 🗓️ Agrupamento semanal
      ...
      salesData.forEach((sale) => {
        ...
        const key = `${weekDay} ${day}/${month}` // exemplo: "Seg 03/11"
        grouped[key] = (grouped[key] || 0) + sale.total_amount
      })
    }

    // ☀️ Agrupamento diário (dia mais recente)
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={chartData}
              onClick={(state) => {
                // 🩵 Corrigido: extrai só o nome do dia
                if (viewMode === "week" && state?.activeLabel) {
                  const label = state.activeLabel as string
                  const dayName = label.split(" ")[0] // ex: "Seg 03/11" → "Seg"
                  setSelectedWeekDay(dayName)
                }
              }}
            >
